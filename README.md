...
-   Breaks down the RSN/WPA security of each AP (WPA version, AKM suites, pairwise/group ciphers, enterprise flag, and management-frame protection) from beacons into the `Security` table.
-   Captures EAP-MD5 challenge/response pairs for offline cracking (`hashcat -m 4800`) into the `EAPMD5` table.
-   Detects randomized (locally administered) client MAC addresses and fingerprints clients by their probe-request information elements (`ProbeFingerprint` table).
...
-  `Security`: This table stores the RSN/WPA security details parsed from the access point beacons and probe responses. Each row is keyed by the access point `bssid` (a foreign key referencing the `AP` table) and stores the negotiated `wpa_version` (WPA2, WPA3, WPA2/WPA3 transition, OWE), the authentication and key management suites (`akm_suites`, e.g. PSK, SAE, 802.1X), the `pairwise_ciphers` and `group_cipher` (CCMP-128, GCMP-256, TKIP, etc.), an `enterprise` flag set when an 802.1X AKM is present, the management-frame-protection state (`pmf`: Required, Capable or Disabled), the raw `rsn_capabilities` bitfield, and the source capture `file`. It complements the management-frame protection columns (`mfpc`, `mfpr`) in the `AP` table, which it also updates from beacons.
...
-  `ProbeFingerprint`: This table fingerprints client devices by the ordered set of information elements (tags) they include in their probe requests, which is fairly characteristic of a device model/OS. Each row stores the client `mac` (a foreign key referencing the `Client` table), a `fingerprint` hash, the ordered `ie_order` list of element IDs, and the source `file`. It uses the combination of `mac` and `fingerprint` as a primary key.
...
-  `SecurityAP`: This view selects the BSSID and SSID of the access point together with the `wpa_version`, the `akm_suites`, the `pairwise_ciphers`, the `group_cipher`, the `enterprise` flag, and the management-frame protection columns (`mfpc`, `mfpr`). It joins the `Security` and `AP` tables on the BSSID and orders the results by BSSID.
