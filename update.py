...
def check_for_update(VERSION):
    ...
    if latest_release_tag:
        ...
        latest_release_tag_number = re.search(r'(\d+(\.\d+)+)', latest_release_tag).group(1)
        ...
        if latest_release_tag_number > current_number:
            user_choice = input("A new version is available (v"+latest_release_tag_number+"). Do you want to update (Y/n)?: ").strip().lower() or "y"
            if user_choice in ("", "y", "Y"):
                print("Updating...")
                update_process = subprocess.Popen(["git", "pull"], cwd=script_dir)
                update_process.wait()  # Wait for the Git pull operation to complete
